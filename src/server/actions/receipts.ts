 ...
 *
 * The gateway does not hand us the receipt at settlement time — its invoicing
 * company generates it asynchronously and PayPlus reports it afterwards, behind
 * a **signed, expiring link**. So nothing is stored: the document is looked up
 * when a buyer (or an admin) asks for it, and the link they get is fresh.
 *
 ...
 * `settleOrder`: probing a stranger's purchase against PayPlus would turn this
...
export async function getPurchaseReceipt(purchaseId: string): Promise<ReceiptResult> {
  ...
  // Every call is a round trip to PayPlus, so it is throttled per user rather
