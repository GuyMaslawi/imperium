...
export function DiscordJoin({
  ...
  const claim = () => {
    ...
    startTransition(async () => {
      ...
      else setError(result.error ?? t("אירעה שגיאה"));
    });
  };
  ...
  return (
    <div className="panel-gold rounded-xl p-5">
      ...
      <p className="text-sm leading-relaxed text-bone/90">
        {t("פעם אחת בלבד, לכל אימפריה:")}{" "}
        <b className="nums text-cyan-300">{DISCORD_JOIN_DIAMONDS}</b>{" "}
        {t("יהלומים על הצטרפות לערוץ. אנחנו לא בודקים — סומכים עליכם. מתנת פתיחה, כל עוד הערוץ צעיר.")}
      </p>
      ...
      {done ? (
        <p className="mt-4 flex items-center gap-2 rounded-lg border border-emerald-500/40 bg-emerald-950/40 px-4 py-3 text-sm font-bold text-emerald-300">
          ...
          {t("המתנה נאספה. תודה שהצטרפת — נתראה בערוץ.")}
        </p>
      ) : !url ? (
        <p className="mt-4 rounded-lg border border-border-subtle bg-black/30 px-4 py-3 text-sm text-zinc-400">
          {t("המתנה תיפתח לאיסוף ברגע שהערוץ יעלה לאוויר.")}
        </p>
      ) : (
        <>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <DiscordLink url={url} variant="pill" label={t("1. פתחו את הערוץ")} />
              ...
              >
                {pending ? (
                  ...
                  <>
                    ...
                    {t("2. אספו {count} יהלומים", { count: DISCORD_JOIN_DIAMONDS })}
                  </>
                )}
              </button>
              {!pending && (
